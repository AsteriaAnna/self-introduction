...
class Logger {
  ...
  private maskSensitiveData(data: Record<string, unknown>): Record<string, unknown> {
    const sensitiveKeys = ['password', 'pwd', 'token', 'secret', 'phone', 'mobile', 'idCard', '身份证', '密码']
  ...
  private log(level: LogLevel, message: string, context?: string, details?: Record<string, unknown>, stackTrace?: string) {
...
export function createModuleLogger(moduleName: string): ModuleLogger {
  return {
    ...
    warn: (message: string, details?: Record<string, unknown>) => logger.warn(message, moduleName, details),
    error: (message: string, details?: Record<string, unknown>, error?: Error) => logger.error(message, moduleName, details, error),
